...
*/
{
	var fileExchangeHandler = (function (){
		...
		var SHA1_MD5_SPLITTER = new RegExp(/(\b[a-zA-Z0-9]+)\b\s+\*(.+)/);
...
*/
{
	...
	function onDOpusCmdMTHManagerStart(cmdData) {
		...
		var tsStart = getTS();
		var itercnt = 0, itermax = Math.round(2 * command.maxwait / (sleepdur||1));
	...
	function getManagerCommand(cmdData) {
		...
		var VALID_SWITCHES = {
			'CALCULATION_SIMULATION' : { filter: filters.PUBLIC.fnFilterAcceptAnyFile,             action: actions.PUBLIC.fnActionCalculateOnly },
			...
			'EXPORT_TO_FILE'         : { filter: filters.PUBLIC.fnFilterRejectAnyFile,             action: actions.PUBLIC.fn_NOT_IMPLEMENTED_YET },
			'IMPORT_FROM_FILE'       : { filter: filters.PUBLIC.fnFilterRejectAnyFile,             action: actions.PUBLIC.fn_NOT_IMPLEMENTED_YET },
			'VERIFY_FROM_FILE'       : { filter: filters.PUBLIC.fnFilterRejectAnyFile,             action: actions.PUBLIC.fn_NOT_IMPLEMENTED_YET }
		};
		for (var sw in VALID_SWITCHES) {
			if (cargs.got_arg[sw]) {
				...
				logger.snormal('%s -- Selected Action: %s, Using Filter: %s, Action: %s, Collection: %s', fnName, sw, ma.filterName, ma.actionName, ma.collName);
				return ma;
			}
		}
	...
	function addFilesToCollection(filepathsArray, collectionName) {
		...
		util.cmd.RunCommand('Delete FORCE QUIET "coll://' + collectionName + '"');
	...
	var ADS = (function (){
		...
		function initCacheIfNecessary() {
			...
			if (util.sv.Exists('cache')) {
				logger.sverbose('%s -- Cache already initialized - Current count: %d', fnName, util.sv.Get('cache').count);
				return;
			}
			if (!util.sv.Exists('cache') || !util.sv.Get('cache') || typeof util.sv.Get('cache') === 'undefined') {
				clearCache();
			}
			logger.sforce('%s -- Cache initialized - Current count: %d', fnName, util.sv.Get('cache').count);
		};
		/**
		...
		return {
			...
			save: function (oItem, oCachedItem) {
				...
				if (CACHE_ENABLED) {
					// @ts-ignore
					util.sv.Get('cache')(rp) = enrichWithCacheFields(oCachedItem);
...
*/
{
	...
	// ManagerCommand
	{
		 ...
		 * @see actions.PUBLIC
		 */
		function ManagerCommand(command, recurse, maxcount, maxwait, fnFilter, fnAction, collectionName) {
	...
	// CommandResults
	{
		...
		function CommandResults(oHashedItemsColl, rootPath, algorithm) {
			...
			var ts       = getTS(),
				items    = oHashedItemsColl,
				oSuccess = items.getSuccessItems(),
				oSkipped = items.getSkippedItems(),
				oError   = items.getErrorItems();
			...
			if (EXPORT_EXTENDED_DATA && !EXPORT_EXTRA_COMPACT) {
				...
				var oEarliestItem = items.getEarliestItem(),
					oLatestItem   = items.getLatestItem(),
				...
				this.Latest_File_Name               = oLatestItem.fullpath;
				this.Latest_File_DateTime_Compact   = oLatestItem.mod_date; // already formatted
